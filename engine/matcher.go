...
func logTrade(bid, ask *models.Order, price float64, quantity int, tx *sql.Tx) error {
	...
	if err := db.CreateTradeTx(trade, tx); err != nil {
	...
	log.Printf("Trade: %s, Price: %.2f, Quantity: %d", trade.Symbol, trade.Price, trade.Quantity)
	return nil
}
...
func (ob *OrderBook) MatchOrders(newOrder *models.Order) error {
    ...
    if err := db.CreateOrderTx(newOrder, tx); err != nil {
        log.Printf("Failed to create order %v: %v", newOrder, err)
        return err
    }
    ...
    log.Printf("Processing order %d, type: %s, matched: %v, bids: %d, asks: %d", newOrder.ID, newOrder.Type, matched, len(bids), len(asks))
